...
import { Package, Heart, CreditCard, Settings, ShoppingBag, ArrowRight, Star } from 'lucide-react';
  ...
  return (
    <CustomerGuard>
      <div className="min-h-screen bg-[#fafbfc] flex flex-col relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-violet-50/50 rounded-full blur-[100px] -z-10" />
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="grid lg:grid-cols-4 gap-12">
              ...
              <aside className="lg:col-span-1 space-y-3">
                 {[
                   { icon: Package, label: 'Acquisitions', href: '/orders', active: true },
                   { icon: ShoppingBag, label: 'Marketplace', href: '/products' },
                   { icon: CreditCard, label: 'Finance Hub', href: '#' },
                   { icon: Star, label: 'Saved Pieces', href: '#' },
                   { icon: Settings, label: 'Preference', href: '#' },
                 ].map((nav, i) => (
                   <Link key={i} href={nav.href} className={`flex items-center gap-4 p-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-[0.15em] transition-all duration-300 ${nav.active ? 'bg-slate-950 text-white shadow-2xl shadow-slate-200 translate-x-2' : 'bg-white/60 backdrop-blur-md text-slate-400 hover:bg-white hover:text-indigo-600 border border-slate-100'}`}>
                      <nav.icon className={`w-5 h-5 ${nav.active ? 'text-indigo-400' : ''}`} />
                      {nav.label}
                   </Link>
                 ))}
              </aside>
              ...
              <div className="lg:col-span-3 space-y-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                   {[
                     ...
                     { label: 'Active Cart', value: '3 items', color: 'from-violet-600 to-fuchsia-500', icon: ShoppingBag },
